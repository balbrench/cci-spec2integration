<!-- Source: https://learn.microsoft.com/en-us/azure/logic-apps/connectors/informix -->
...
| **Get rows** | Get all the rows in the specified Informix table by running an Informix `SELECT *` statement. | **Table name**: The name for the Informix table that you. want <br><br>To add other parameters to this action, add them from the **Advanced parameters** list. For more information, see the [connector reference article](https://learn.microsoft.com/en-us/connectors/informix/). |
...
* To connect with on-premises Informix databases, you need to [download and install the on-premises data gateway](https://learn.microsoft.com/en-us/azure/logic-apps-gateway-install) on a local computer and then [create an Azure data gateway resource in the Azure portal](https://learn.microsoft.com/en-us/azure/logic-apps-gateway-connection).

* The Consumption or Standard logic app workflow where you need access to your Informix database.

  The Informix connector provides only actions, so your workflow must start with an existing trigger that best suits your scenario. This example uses the [**Recurrence** trigger](https://learn.microsoft.com/en-us/azure/../connectors/connectors-native-recurrence).
...
1. On the designer, follow these [general steps to add the **Informix** action that you want](https://learn.microsoft.com/en-us/azure/add-trigger-action-workflow.md?tabs=consumption#add-action) to your workflow.
...
1. On the designer, follow these [general steps to add the **Informix** action that you want](https://learn.microsoft.com/en-us/azure/add-trigger-action-workflow.md?tabs=standard#add-action) to your workflow.
...
1. Specify the following connection information:
   ...
   * **Cloud database**
     ...
     ![Screenshot shows connection pane with example details for Informix cloud database.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/connectors/media/informix/connection-cloud-database.png)

   * **On-premises database**

     ![Screenshot shows connection pane with example details for Informix on-premises database.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/connectors/media/informix/connection-on-premises-database.png)
...
- When possible, avoid methods that employ a username and password or tokens.
  ...
  [!INCLUDE [guidance-authentication-flows](https://learn.microsoft.com/en-us/azure/includes/guidance-authentication-flows)]

- Make sure that you secure and protect sensitive and personal data.

  [!INCLUDE [secrets-guidance](https://learn.microsoft.com/en-us/azure/includes/secrets-guidance)]
...
1. [Follow the general steps to view the latest workflow run and the information for each step in the workflow](https://learn.microsoft.com/en-us/azure/view-workflow-status-run-history.md?tabs=consumption#review-run-history).
...
1. [Follow the general steps to view the latest workflow run and the information for each step in the workflow](https://learn.microsoft.com/en-us/azure/view-workflow-status-run-history.md?tabs=standard#review-run-history).
...
![Screenshot shows outputs from action named Get rows.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/connectors/media/informix/get-rows-outputs.png)
