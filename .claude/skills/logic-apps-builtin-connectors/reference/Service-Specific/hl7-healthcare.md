<!-- Source: https://learn.microsoft.com/en-us/azure/logic-apps/connectors/integrate-healthcare-systems -->

# Build HL7 <!--and MLLP-->healthcare integrations with Standard workflows in Azure Logic Apps (preview)

[!INCLUDE [logic-apps-sku-standard](https://learn.microsoft.com/en-us/azure/../../includes/logic-apps-sku-standard)]
...
This guide shows how to use the **HL7** <!--and **MLLP**--> connector operations in Standard workflows. In this guide, the examples use the Azure portal, but you can create Standard workflows with these operations in [Visual Studio Code](https://learn.microsoft.com/en-us/azure/logic-apps/create-single-tenant-workflows-visual-studio-code).
...
![Conceptual diagram shows common HL7 message types such as ADT, ORM, ORU, EFT, and EPR.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/connectors/media/integrate-healthcare-systems/hl7-messages.png)
...
- An [integration account resource](https://learn.microsoft.com/en-us/azure/logic-apps/enterprise-integration/create-integration-account) to define and store artifacts for use with enterprise integrations and B2B workflows.

  - Both your integration account and logic app resource must exist in the same Azure subscription and Azure region.

  - Before you start working with HL7 or MLLP operations, you must [link your Standard logic app to your integration account](https://learn.microsoft.com/en-us/azure/logic-apps/enterprise-integration/create-integration-account?tabs=standard#link-account).

<!--### [HL7](#tab/hl7)-->

- HL7 flat file schema and all the BizTalk schemas referenced by your HL7 schema.

  You must [upload all these schemas to your integration account](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-enterprise-integration-schemas?tabs=standard#add-schema-to-integration-account). However, unlike BizTalk Server, you don't need to upload the following common schemas:
  ...
  ![Screenshot shows integration account with HL7 schemas.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/connectors/media/integrate-healthcare-systems/integration-schemas.png) 

- A Standard logic app resource and workflow to use for your healthcare integration. See [Create Standard logic app workflows in Azure Logic Apps](https://learn.microsoft.com/en-us/azure/logic-apps/create-single-tenant-workflows-azure-portal).
...
- A Standard logic app resource that uses the hybrid deployment option.
  ...
  - On your logic app, you need to open custom TCP/IP ports to exchange MLLP messages. Azure Logic Apps requires these ports and acts as a listener on the configured port.
    ...
    For more information, see [Control inbound traffic to your logic app](https://learn.microsoft.com/en-us/azure/logic-apps/create-standard-workflows-hybrid-deployment?tabs=azure-portal).
...
1. If your workflow doesn't have a trigger, follow the [general steps](https://learn.microsoft.com/en-us/azure/logic-apps/create-workflow-with-trigger-or-action?tabs=standard#add-trigger) to add the trigger that best suits your scenario.

   This example continues with the **Request** trigger named **When an HTTP request is received**, for example:

   ![Screenshot shows Azure portal, Standard workflow designer, and Request trigger.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/connectors/media/integrate-healthcare-systems/request-trigger.png)

1. Follow the [general steps](https://learn.microsoft.com/en-us/azure/logic-apps/create-workflow-with-trigger-or-action?tabs=standard#add-action) to add the **HL7** built-in connector action named **Encode HL7**.
...
1. On the designer, select the added action, if not selected. In the action pane, provide the following information:
   ...
   ![Screenshot shows Encode HL7 action parameters.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/connectors/media/integrate-healthcare-systems/encode-hl7.png)
...
1. If your workflow doesn't have a trigger, follow the [general steps](https://learn.microsoft.com/en-us/azure/logic-apps/create-workflow-with-trigger-or-action?tabs=standard#add-trigger) to add the trigger that best suits your scenario.
   ...
   ![Screenshot shows the Azure portal, Standard workflow designer, and Request trigger.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/connectors/media/integrate-healthcare-systems/request-trigger.png)

1. Follow the [general steps](https://learn.microsoft.com/en-us/azure/logic-apps/create-workflow-with-trigger-or-action?tabs=standard#add-action) to add the **HL7** built-in connector action named **Decode HL7**.
...
1. On the designer, select the added action, if not selected. In the action pane, provide the following information:
   ...
   ![Screenshot shows the Decode HL7 action parameters.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/connectors/media/integrate-healthcare-systems/decode-hl7.png)
...
1. Follow the [general steps](https://learn.microsoft.com/en-us/azure/logic-apps/create-workflow-with-trigger-or-action.md?tabs=standard#add-trigger) to add the **MLLP** built-in trigger named **Receive message**.
...
1. On the designer, select the added trigger, if not selected. In the connection pane, provide the following information:
   ...
   ![Screenshot shows Standard workflow designer and connection pane for MLLP trigger.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/connectors/media/integrate-healthcare-systems/connection-mllp-trigger.png)
...
1. If you workflow doesn't have a trigger, follow the [general steps](https://learn.microsoft.com/en-us/azure/logic-apps/create-workflow-with-trigger-or-action?tabs=standard#add-trigger) to add the trigger that best suits your scenario.

   This example continues with the **Request** trigger named **When an HTTP request is received**, for example:

   ![Screenshot shows the Azure portal, Standard workflow, designer, and Request trigger.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/connectors/media/integrate-healthcare-systems/request-trigger.png)

1. Follow the [general steps](https://learn.microsoft.com/en-us/azure/logic-apps/create-workflow-with-trigger-or-action.md?tabs=standard#add-action) to add the **MLLP** built-in action named **Send message**.
...
1. On the designer, select the added action, if not selected. In the connection pane, provide the following information:
   ...
   ![Screenshot shows Standard workflow designer and connection pane for MLLP action.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/connectors/media/integrate-healthcare-systems/connection-mllp-action.png)
...
1. On the designer, select the added action, if not selected. In the action pane, provide the following information:
   ...
   ![Screenshot shows Send message for MLLP parameters.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/connectors/media/integrate-healthcare-systems/mllp-send-message.png)
...
- [Create Standard logic app workflows in Azure Logic Apps](https://learn.microsoft.com/en-us/azure/logic-apps/create-single-tenant-workflows-azure-portal)
- [Create Standard logic app workflows for hybrid deployment on your own infrastructure](https://learn.microsoft.com/en-us/azure/logic-apps/create-standard-workflows-hybrid-deployment)
