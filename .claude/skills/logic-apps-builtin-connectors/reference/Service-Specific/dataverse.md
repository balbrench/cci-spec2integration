<!-- Source: https://learn.microsoft.com/en-us/azure/logic-apps/connectors/dataverse -->
...
To automate tasks that work with your [Microsoft Dataverse database](/power-apps/maker/data-platform/data-platform-intro), you can use the [Microsoft Dataverse connector](https://learn.microsoft.com/en-us/connectors/commondataserviceforapps/) with workflows in [Azure Logic Apps](https://learn.microsoft.com/en-us/azure/logic-apps-overview).
...
For technical information based on the connector's Swagger description, such as operations, limits, and other details, see the [managed connector reference page](https://learn.microsoft.com/en-us/connectors/commondataserviceforapps/).
...
- A [Dataverse Data Service environment and database](/power-platform/admin/environments-overview), which is where your organization stores, manages, and shares business data in a Dataverse database.
  ...
  - [Learn: Create and manage Dataverse environments](https://learn.microsoft.com/en-us/training/modules/create-manage-environments/)
...
- Basic knowledge about Azure Logic Apps along with the Consumption or Standard logic app resource and workflow from where you want to access your Dataverse database. To use a Dataverse trigger, you need a blank workflow. To use a Dataverse action, you need a workflow that starts with any trigger appropriate for your scenario.
  ...
  - [Create an example Consumption logic app workflow](https://learn.microsoft.com/en-us/azure/quickstart-create-example-consumption-workflow)
  - [Create an example Standard logic app workflow](https://learn.microsoft.com/en-us/azure/create-single-tenant-workflows-azure-portal)
...
1. Follow the [general steps](https://learn.microsoft.com/en-us/azure/add-trigger-action-workflow.md#add-trigger) to add the appropriate **Microsoft Dataverse** trigger for your scenario.

   This example continues with the trigger named [**When a row is added, modified or deleted**](https://learn.microsoft.com/en-us/connectors/commondataserviceforapps/#when-a-row-is-added,-modified-or-deleted).
...
1. In the trigger information box, provide the necessary parameter values.
   ...
   ![Screenshot shows Consumption workflow designer and example trigger.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/connectors/media/dataverse/dataverse-trigger-example.png)
...
1. Follow the [general steps](https://learn.microsoft.com/en-us/azure/add-trigger-action-workflow.md#add-action) to add the appropriate **Microsoft Dataverse** action for your scenario.

   This example continues with the action named [**Add a new row**](https://learn.microsoft.com/en-us/connectors/commondataserviceforapps/#add-a-new-row).
...
1. In the action information box, provide the necessary parameter values.
   ...
   ![Screenshot shows Consumption workflow designer and example action.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/connectors/media/dataverse/dataverse-action-example.png)
...
1. On the designer, in the action, open the **Advanced parameters** list, and select the **Filter rows** parameter.
   ...
   ![Screenshot shows workflow and parameter named Filter rows.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/connectors/media/dataverse/dataverse-action-filter-rows.png)

1. In the **Filter rows** parameter that now appears in the action, enter an ODATA query expression, for example:

   `statuscode eq 1`

   ![Screenshot shows Standard workflow and parameter named Filter rows with ODATA query.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/connectors/media/dataverse/dataverse-action-filter-rows-query.png)

For more information, see the following documentation:

- [List rows](https://learn.microsoft.com/en-us/connectors/commondataserviceforapps/#list-rows)
...
1. On the designer, in the action, open the **Advanced parameters** list, and select the **Sort By** parameter.
   ...
   ![Screenshot shows workflow, a Dataverse action, and Sort By parameter.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/connectors/media/dataverse/dataverse-action-sort-by.png)

1. In the **Sort By** parameter that now appears in the action, enter the column name to use for sorting, such as **name**:

   ![Screenshot shows workflow, a Dataverse action, and Sort By parameter with column name.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/connectors/media/dataverse/dataverse-action-sort-by-column.png)
...
![Screenshot shows workflow code view, Add a new row action, and new tasks row associated with IDs and lookup types.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/connectors/media/dataverse/add-new-row-task-properties.png)
...
1. In the Chrome browser, find the callback registration that you want to delete.
   ...
   1. Filter on the triggering entity's logical name `entityname` and the notification event that matches your logic app workflow (message). Each event type is mapped to the message integer as follows:
      ...
      ![Screenshot shows browser window and OData URI in address bar.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/connectors/media/dataverse/find-callback-registrations.png)
