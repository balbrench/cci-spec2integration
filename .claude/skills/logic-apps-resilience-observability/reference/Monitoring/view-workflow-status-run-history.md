<!-- Source: https://learn.microsoft.com/en-us/azure/logic-apps/view-workflow-status-run-history -->
...
For real-time event monitoring and richer debugging, you can set up diagnostics logging for your logic app workflow by using [Azure Monitor logs](https://learn.microsoft.com/en-us/azure/azure-monitor/overview). This Azure service helps you monitor your cloud and on-premises environments so that you can more easily maintain their availability and performance. You can then find and view events, such as trigger events, run events, and action events. By storing this information in [Azure Monitor logs](https://learn.microsoft.com/en-us/azure/azure-monitor/logs/data-platform-logs), you can create [log queries](https://learn.microsoft.com/en-us/azure/azure-monitor/logs/log-query-overview) that help you find and analyze this information. You can also use this diagnostic data with other Azure services, such as Azure Storage and Azure Event Hubs. For more information, see [Monitor logic apps by using Azure Monitor](https://learn.microsoft.com/en-us/azure/logic-apps/monitor-workflows-collect-diagnostic-data).
...
1. On your logic app menu, select **Overview**. On the **Overview** page, select **Trigger history**.
   ...
   ![Screenshot shows Azure portal, Consumption workflow, and Overview page with selected tab named Trigger history.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/view-workflow-status-run-history/trigger-history-consumption.png)
   ...
   ![Screenshot shows Overview page with Consumption workflow and multiple trigger attempts for different items.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/view-workflow-status-run-history/triggers-history-consumption.png)
...
1. To view information about a specific trigger attempt, select that trigger event.
   ...
   ![Screenshot shows Consumption workflow trigger history and selected entry.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/view-workflow-status-run-history/select-trigger-event-consumption.png)
   ...
   ![Screenshot shows selected Consumption workflow trigger history information.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/view-workflow-status-run-history/trigger-details-consumption.png)
...
1. On the workflow menu, under **Tools**, select **Run history**. On the **Run history** page, select **Trigger history**.
   ...
   ![Screenshot shows Azure portal, Standard workflow, and Overview page with selected tab named Trigger history.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/view-workflow-status-run-history/trigger-history-standard.png)

   Under **Trigger history**, all trigger attempts appear. Each time the trigger successfully fires, Azure Logic Apps creates an individual workflow instance and runs that instance. By default, each instance runs in parallel so that no workflow has to wait before starting a run. If your workflow triggers for multiple events or items at the same time, a trigger entry appears for each item with the same date and time. 

   ![Screenshot shows Standard workflow and Run history page with tab named Trigger history, which shows multiple trigger attempts for different items.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/view-workflow-status-run-history/triggers-history-standard.png)
...
1. To view information about a specific trigger attempt, select the identifier for that trigger attempt.
   ...
   ![Screenshot shows Standard workflow trigger entry selected.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/view-workflow-status-run-history/select-trigger-event-standard.png)
...
1. Check the trigger's inputs to confirm that they appear as you expect. On the **History** pane, under **Inputs link**, select the link, which opens the **Inputs** pane.
   ...
   ![Screenshot shows Standard workflow trigger inputs.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/view-workflow-status-run-history/trigger-inputs-standard.png)
...
1. Check the triggers outputs, if any, to confirm that they appear as you expect. On the **History** pane, under **Outputs link**, select the link, which opens the **Outputs** pane.
   ...
   ![Screenshot shows Standard workflow trigger outputs.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/view-workflow-status-run-history/trigger-outputs-standard.png)
...
1. On your logic app menu, select **Overview**. On the **Overview** page, select **Runs history**.
   ...
   ![Screenshot shows Consumption workflow and Overview page with selected tab named Runs history.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/view-workflow-status-run-history/run-history-consumption.png)
   ...
   | **Running** | The run was triggered and is in progress. However, this status can also appear for a run that is throttled due to [action limits](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-limits-and-config) or the [current pricing plan](https://azure.microsoft.com/pricing/details/logic-apps/). <br><br>**Tip**: If you set up [diagnostics logging](https://learn.microsoft.com/en-us/azure/logic-apps/monitor-workflows-collect-diagnostic-data), you can get information about any throttle events that happen. |
   | **Succeeded** | The run succeeded. If any action failed, a subsequent action in the workflow handled that failure. |
   | **Timed out** | The run timed out because the current duration exceeded the run duration limit, which is controlled by the [setting named **Run history retention in days**](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-limits-and-config#run-duration-retention-limits). The run duration is calculated by using the run's start time and run duration limit at that start time. <br><br>**Note**: If the run duration also exceeds the current *run history retention limit*, which is also controlled by the [setting named **Run history retention in days**](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-limits-and-config#run-duration-retention-limits), the run is cleared from the run history by a daily cleanup job. Whether the run times out or completes, the retention period is always calculated by using the run's start time and *current* retention limit. So, if you reduce the duration limit for an in-flight run, the run times out. However, the run either stays or is cleared from the run history based on whether the run duration exceeded the retention limit. |
...
1. To review the steps and other information for a specific run, under **Runs history**, select that run. If the list shows many runs, and you can't find the entry that you want, try filtering the list.
   ...
   ![Screenshot shows a selected Consumption workflow run.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/view-workflow-status-run-history/select-run-consumption.png)

   The run history page opens and shows the status for each step in the selected run, for example:

   ![Screenshot shows Consumption workflow run history with each action in the run.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/view-workflow-status-run-history/run-history-pane-consumption.png)
...
1. To view the information in list form, on the run history toolbar, select **Run details**.
   ...
   ![Screenshot shows run details for each step in the Consumption workflow.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/view-workflow-status-run-history/run-details-consumption.png)

   For example, you can get the run's **Correlation Id** property, which you might need when you use the [REST API for Logic Apps](https://learn.microsoft.com/en-us/rest/api/logic).
...
1. To get more information about a specific step, select either option:
   ...
   * On the run history page, select a step to open a pane that shows the inputs, outputs, and any errors that happened in that step.
     ...
     ![Screenshot shows Consumption workflow run history page with selected failed example step plus inputs, outputs, and errors for the failed step.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/view-workflow-status-run-history/inputs-outputs-errors-consumption.png)
   ...
   * On the run history page toolbar, select **Run details**. In the **Logic app run details** pane that opens, select the step that you want, for example:
     ...
     ![Screenshot shows Consumption workflow, and pane named Logic app run details. The pane shows the selected example failed step.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/view-workflow-status-run-history/select-failed-step-consumption.png)
   ...
   > [hide inputs and outputs in the workflow run history](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-securing-a-logic-app#obfuscate)
...
You can view run history only for stateful workflows, not stateless workflows. To enable run history for a stateless workflow, see [Enable run history for stateless workflows](https://learn.microsoft.com/en-us/azure/logic-apps/create-single-tenant-workflows-azure-portal#enable-run-history-stateless).
...
1. On your workflow menu, under **Tools**, select **Run history**. On the **Run history** page, select **Run history**.
   ...
   ![Screenshot shows Standard workflow, run history page, and selected tab named Run history.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/view-workflow-status-run-history/run-history-standard.png)
   ...
   | **Running** | ![Running icon][running-icon] | The run was triggered and is in progress. However, this status can also appear for a run that is throttled due to [action limits](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-limits-and-config) or the [current pricing plan](https://azure.microsoft.com/pricing/details/logic-apps/). <br><br>**Tip**: If you set up [diagnostics logging](https://learn.microsoft.com/en-us/azure/logic-apps/monitor-workflows-collect-diagnostic-data), you can get information about any throttle events that happen. |
...
1. To review the steps and other information for a specific run, on the **Run history** tab, select that run. If the list shows many runs, and you can't find the entry that you want, try filtering the list.
   ...
   ![Screenshot shows selected Standard workflow run.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/view-workflow-status-run-history/select-run-standard.png)

   The run history page opens and shows the status for each step in the selected run, for example:

   ![Screenshot shows Standard workflow and each action in the selected run.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/view-workflow-status-run-history/run-history-pane-standard.png)
...
1. To get more information about a specific step, on the run history page, select a step to open a pane that shows the inputs, outputs, and any errors that happened in that step.
   ...
   ![Screenshot shows Standard workflow with failed step inputs.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/view-workflow-status-run-history/failed-action-inputs-standard.png)

   The following screenshot shows the outputs from the failed step.

   ![Screenshot shows Standard workflow with failed step outputs.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/view-workflow-status-run-history/failed-action-outputs-standard.png)
...
- By default, only Consumption workflows and Standard stateful workflows, which record and store run history, are supported. To use these capabilities with a stateless Standard workflow, enable stateful mode. For more information, see [Enable run history for stateless workflows](https://learn.microsoft.com/en-us/azure/logic-apps/create-single-tenant-workflows-azure-portal#enable-run-history-stateless) and [Enable stateful mode for stateless connectors](https://learn.microsoft.com/en-us/azure/connectors/enable-stateful-affinity-built-in-connectors).
...
You can rerun only stateful workflows, not stateless workflows. To enable run history for a stateless workflow, see [Enable run history for stateless workflows](https://learn.microsoft.com/en-us/azure/logic-apps/create-single-tenant-workflows-azure-portal#enable-run-history-stateless).
...
To get alerts based on specific metrics or exceeded thresholds in your workflow, set up your logic app resource with [alerts in Azure Monitor](https://learn.microsoft.com/en-us/azure/azure-monitor/alerts/alerts-overview). For more information, see [Metrics in Azure](https://learn.microsoft.com/en-us/azure/azure-monitor/data-platform).

To set up alerts without using [Azure Monitor](https://learn.microsoft.com/en-us/azure/azure-monitor/logs/log-query-overview), follow these steps, which apply to both Consumption and Standard logic app resources:
...
1. On the **Create an alert rule** page, from the **Signal name** list, select the signal for which you want to get an alert.
   ...
   1. Under **When to evaluate**, set up the schedule for checking the condition:
      ...
      ![Screenshot shows Consumption logic app resource with alert condition.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/view-workflow-status-run-history/set-up-alert-condition.png)

1. When you're ready, select **Review + Create**.

For general information, see [Create an alert rule from a specific resource - Azure Monitor](https://learn.microsoft.com/en-us/azure/azure-monitor/alerts/alerts-create-new-alert-rule#create-or-edit-an-alert-rule-in-the-azure-portal).
