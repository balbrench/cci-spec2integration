<!-- Source: https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-enterprise-integration-b2b -->
...
- The logic app resource and blank workflow where you want to build the B2B integration in this example.
  ...
  - [Create a Consumption logic app workflow using the Azure portal](https://learn.microsoft.com/en-us/azure/logic-apps/quickstart-create-example-consumption-workflow)

  - [Create a Standard logic app workflow using the Azure portal](https://learn.microsoft.com/en-us/azure/logic-apps/create-single-tenant-workflows-azure-portal)

- An [integration account resource](https://learn.microsoft.com/en-us/azure/logic-apps/enterprise-integration/create-integration-account) to define and store artifacts for enterprise integration and B2B workflows.

  - Both the integration account and logic app resource must exist in the same Azure subscription and Azure region.

  - Defines at least two [trading partners](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-enterprise-integration-partners) in your integration account. The definitions for both partners must use the same *business identity* qualifier, such as AS2, X12, EDIFACT, or RosettaNet.

  - Defines an [AS2 agreement and X12 agreement](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-enterprise-integration-agreements) between the trading partners that participate in your workflow. Each agreement requires a host partner and a guest partner.
...
To start the workflow in this example, add the [**Request** trigger](https://learn.microsoft.com/en-us/azure/connectors/connectors-native-reqres).

1. In the [Azure portal](https://portal.azure.com), open your logic app resource.

1. In the designer, open your workflow. Follow these [general steps](https://learn.microsoft.com/en-us/azure/logic-apps/add-trigger-action-workflow#add-trigger) to add the **Request** built-in trigger named **When an HTTP request is received** to your workflow.
...
1. Leave the trigger's **Request Body JSON Schema** parameter empty because the trigger receives X12 messages in flat file format.
   ...
   ![Screenshot shows Azure portal, workflow designer, and Request trigger parameters.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-enterprise-integration-b2b/request-trigger.png)

1. Save your workflow. On the designer toolbar, select **Save**.

   This step generates the **HTTP URL**, which you later use to send a request that triggers the workflow.

   ![Screenshot shows workflow designer, Request trigger parameters, and generated URL for Request trigger.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-enterprise-integration-b2b/request-trigger-generated-url.png)
...
Follow these steps to add the [**AS2 (v2)** decode action](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-enterprise-integration-as2).

1. Under the **Request** trigger, follow these [general steps](https://learn.microsoft.com/en-us/azure/logic-apps/add-trigger-action-workflow#add-action) to add the following **AS2 (v2)** action, based on your workflow type:
...
1. In the action's **Message to decode** parameter, provide the message content to decode.
   ...
   - To choose output from previous operations, follow these steps for the dynamic content list:
     ...
     1. Under **When an HTTP request is received**, select **Body**, for example:
        ...
        ![Screenshot shows workflow designer, Request trigger parameters, and dynamic content list with Body parameter selected.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-enterprise-integration-b2b/select-trigger-output-body.png)
...
1. In the action's **Message headers** parameter, enter any headers that the AS2 action requires. You can find these values in the `headers` property from the **Request** trigger output by following these steps:
   ...
   1. In the **Message headers** section, select **Switch Message headers to text mode**:
      ...
      ![Screenshot shows AS2 decode action with Switch Message headers to text mode selected.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-enterprise-integration-b2b/switch-text-mode.png)
   ...
   1. In the editor box, enter the following expression, and select **Add**:
      ...
      ![Screenshot shows the Message headers box with an expression that references the headers in trigger output.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-enterprise-integration-b2b/header-expression.png)
...
1. On the designer, under the **AS2 (v2)** decode action, follow these [general steps](https://learn.microsoft.com/en-us/azure/logic-apps/add-trigger-action-workflow#add-action) to add the **Condition** built-in action.
   ...
   ![Screenshot shows the Condition action with empty paths.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-enterprise-integration-b2b/added-condition-action.png)
...
1. In the right-side **Choose a value** box, enter the value `Expected`.
   ...
   ![Screenshot shows the Condition action with the example condition to evaluate.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-enterprise-integration-b2b/evaluate-condition-expression.png)
...
1. For when the **AS2 (v2)** decode action succeeds, follow these steps:
   ...
   1. To reference the AS2 MDN from the **AS2 (v2)** decode action output, specify the following expressions:
      ...
      ![Screenshot shows Response action and resolved expressions that access the AS2 MDN.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-enterprise-integration-b2b/response-success-resolved-expression.png)
...
1. On the designer, under the **Response** action, follow these [general steps](https://learn.microsoft.com/en-us/azure/logic-apps/add-trigger-action-workflow#add-action) to add the following X12 decode action, based on your workflow type:
   ...
   For more information, see [Exchange X12 messages in B2B workflows with Azure Logic Apps](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-enterprise-integration-x12).
...
1. If the action prompts you to create a connection, provide the following information, and select **Create new**.
   ...
   For more information, see [Decode X12 messages](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-enterprise-integration-x12#decode-x12-messages).
...
- [Connect to SAP systems from Azure Logic Apps](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-using-sap-connector)
- [Monitor, create, and manage SFTP files by using SSH and Azure Logic Apps](https://learn.microsoft.com/en-us/azure/connectors/connectors-sftp-ssh)

To connect your own LOB apps and use these APIs in your workflow, add more actions or [write custom APIs](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-create-api-app).

## Related content

- [Exchange AS2 messages for B2B enterprise integration](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-enterprise-integration-as2)
- [Exchange X12 messages for B2B enterprise integration](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-enterprise-integration-x12)
