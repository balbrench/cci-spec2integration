<!-- Source: https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-enterprise-integration-rosettanet -->
...
The RosettaNet connector has different versions, based on [logic app type and host environment](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-overview#resource-environment-differences).

| Logic app | Environment | Connector version |
|-----------|-------------|-------------------|
| **Consumption** | Multitenant Azure Logic Apps | RosettaNet managed connector, which appears in the connector gallery under **Shared**. The RosettaNet connector provides only actions. You can use any trigger that works for your scenario. For more information, see: <br><br>- [RosettaNet operations](#rosettanet-operations) <br>- [B2B protocol limits for message sizes](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-limits-and-config#b2b-protocol-limits) |
...
- The logic app resource and workflow where you want to use the RosettaNet operations.
  ...
  - [Create a Consumption logic app workflow in Azure Logic Apps](https://learn.microsoft.com/en-us/azure/logic-apps/quickstart-create-example-consumption-workflow)

  - [Create a Standard logic app workflow in Azure Logic Apps](https://learn.microsoft.com/en-us/azure/logic-apps/create-single-tenant-workflows-azure-portal)

- An [integration account resource](https://learn.microsoft.com/en-us/azure/logic-apps/enterprise-integration/create-integration-account) to define and store artifacts for use in enterprise integration and B2B workflows.

  - Both your integration account and logic app resource must exist in the same Azure subscription and Azure region.

  - Defines at least two [trading partners](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-enterprise-integration-partners) that participate in the RosettaNet operation used in your workflow. Both partner definitions must use the same *business identity* qualifier, which is **DUNS**.
...
- Optional [certificates](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-enterprise-integration-certificates) for encrypting, decrypting, or signing the messages that you upload to the integration account. Certificates are required only if you use signing or encryption.
...
1. In the **Add Partner Interface Process** pane, enter the following PIP information:
   ...
   ![Screenshot shows the Azure portal, integration account resource, and RosettaNet PIP page with Add selected. The Add Partner Interface Process pane contains boxes for the name, code, and version.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-enterprise-integration-rosettanet/add-rosettanet-pip.png)
...
1. To view or edit the process configuration, select the PIP. On the **RosettaNet PIP** page toolbar, select **Edit as JSON**.
   ...
   ![Screenshot shows RosettaNet PIP page with a selected PIP and Edit as JSON selected. The JSON editor shows encoded PIP settings.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-enterprise-integration-rosettanet/edit-rosettanet-pip.png)
...
1. On the **Add** pane, enter the following agreement information:
   ...
   ![Screenshot shows Agreements page with Add selected and agreement information.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-enterprise-integration-rosettanet/add-agreement-details.png)
...
1. To specify how the host partner receives messages from the guest partner, in the **Add** pane, select **Receive Settings**.
   ...
   1. Under each selected setting, from the **Certificate** list, select the [certificate](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-enterprise-integration-certificates) from your integration account to use for signing, encryption, or both.

   The following example shows the signing and encryption settings on the **Receive Settings** pane:

   ![Screenshot shows the Receive Settings pane with options for signing and encrypting messages and entering certificates.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-enterprise-integration-rosettanet/add-agreement-receive-details.png)
...
1. To specify how the host partner sends messages to the guest partner, in the **Add** pane, select **Send Settings**.
   ...
   1. Under each selected setting, from the algorithm list, select the algorithm to use. From the **Certificate** list, select the [certificate](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-enterprise-integration-certificates) from your integration account to use for signing, encryption, or both.
   ...
   1. In the **Endpoints** section, specify the required URLs for sending action messages and acknowledgments:
      ...
      ![Screenshot shows the Send Settings pane, with options for signing and encrypting messages and for entering algorithms, certificates, and endpoint URLs.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-enterprise-integration-rosettanet/add-agreement-send-details.png)
...
1. To specify the RosettaNet PIP references for the agreement, in the **Add** pane, select **RosettaNet PIP references**.
   ...
   1. In the **PIP Name** column, open the list, and select the PIP.
      ...
      ![Screenshot that shows a table of PIP information that has one row, which contains default values except the name, MyPIPConfig, which is selected.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-enterprise-integration-rosettanet/add-agreement-pip-details.png)

      Your selection populates the remaining properties, which are based on the PIP in your integration account.

      ![Screenshot shows a table with PIP information, with the row for the PIP named MyPIPConfig showing information.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-enterprise-integration-rosettanet/add-agreement-selected-pip.png)
...
1. Follow these [general steps](https://learn.microsoft.com/en-us/azure/logic-apps/create-workflow-with-trigger-or-action#add-action) to add the RosettaNet decode action to your workflow.
...
1. In the action information pane, provide the following values:
   ...
   ![Screenshot shows the RosettaNet decode action with boxes for the message to decode, headers, and role.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-enterprise-integration-rosettanet/decode-action-details.png)
...
1. Follow these [general steps](https://learn.microsoft.com/en-us/azure/logic-apps/create-workflow-with-trigger-or-action#add-action) to add the RosettaNet encode action to your workflow.
...
1. In the action information pane, provide the following values:
   ...
   ![Screenshot shows the RosettaNet encode action with boxes for the message, partners, PIP information, message type, and role.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-enterprise-integration-rosettanet/encode-action-details.png)

   This action is now set up to encode and send the RosettaNet message to the guest partner.

1. To send the encoded RosettaNet message, follow these [general steps](https://learn.microsoft.com/en-us/azure/logic-apps/create-workflow-with-trigger-or-action#add-action) to add the appropriate action for your scenario. Provide the values that the action need to send the message.
   ...
   ![Screenshot shows the renamed HTTP action with sample URI, header, and body values.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-enterprise-integration-rosettanet/send-rosettanet-message-to-partner.png)
...
1. In your workflow, add the **RosettaNet** action that waits for a response. In the action information pane, provide the following values:
   ...
   ![Screenshot shows the RosettaNet wait for response action with boxes for the body, PIP instance identity, retry count, and role values.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-enterprise-integration-rosettanet/rosettanet-wait-for-response-action.png)
...
1. To handle retry attempts, always put RosettaNet actions to send the encoded message and wait for a response inside an **Until** loop, for example:
   ...
   ![Screenshot of the designer with an Until loop contains actions for encoding and sending messages and for waiting for responses.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-enterprise-integration-rosettanet/rosettanet-loop.png)

## Related content

- [Connectors in Azure Logic Apps](https://learn.microsoft.com/en-us/connectors/connector-reference/connector-reference-logicapps-connectors)
- [Managed connectors in Azure Logic Apps](https://learn.microsoft.com/en-us/azure/connectors/managed)
- [Built-in connectors in Azure Logic Apps](https://learn.microsoft.com/en-us/azure/connectors/built-in)
