<!-- Source: https://learn.microsoft.com/en-us/azure/logic-apps/rules-engine/create-rules-engine-project -->
...
- Download and install the [Microsoft Rules Composer](https://go.microsoft.com/fwlink/?linkid=2274238), which you use to create rulesets, business rules, and vocabularies.
  ...
  - [Create rules with the Microsoft Rules Composer](https://learn.microsoft.com/en-us/azure/logic-apps/create-rules)

  - [Decision management and business logic integration using the Azure Logic Apps Rules Engine](https://learn.microsoft.com/en-us/azure/logic-apps/rules-engine-overview)

- For this release, only Visual Studio Code supports the development experience for an Azure Logic Apps Rules Engine project. To meet the prerequisites for using Visual Studio Code, see [Create a Standard logic app workflow in single-tenant Azure Logic Apps using Visual Studio Code](https://learn.microsoft.com/en-us/azure/create-single-tenant-workflows-visual-studio-code.md#prerequisites).

- The Azure Logic Apps Rules Engine uses the custom code function capability in Visual Studio Code. To meet the prerequisites for using this capability, see [Create and run .NET Framework code from Standard workflows in Azure Logic Apps](https://learn.microsoft.com/en-us/azure/create-run-custom-code-functions.md#prerequisites).
...
1. In the **Azure** window that opens, on the **Workspace** section toolbar, from the **Azure Logic Apps** menu, select **Create new logic app workspace**. 
   ...
   ![Screenshot shows Visual Studio Code, Azure window, Workspace section toolbar, and selected option for Create new logic app workspace.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/rules-engine/media/create-rules-engine-project/create-workspace.png)
...
1. When the **Create new logic app workspace** prompt box appears, provide a name for your workspace:
   ...
   ![Screenshot shows Visual Studio Code with prompt to enter workspace name.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/rules-engine/media/create-rules-engine-project/workspace-name.png)
...
1. When the **Select a project template for your logic app workspace** prompt box appears, select **Logic app with rules engine project**.
   ...
   ![Screenshot shows Visual Studio Code with prompt to select project template for logic app workspace.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/rules-engine/media/create-rules-engine-project/project-template.png)
...
1. Select **Open in current window**.
   ...
   ![Screenshot shows Visual Studio Code with created workspace.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/rules-engine/media/create-rules-engine-project/created-workspace.png)
...
1. From the working directory list that appears, select **Functions** as your current working directory for the new terminal.
   ...
   ![Screenshot shows Visual Studio Code, prompt for current working directory, and selected Functions directory.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/rules-engine/media/create-rules-engine-project/compile-function-project.png)
...
1. In the **Terminal** window, at the command prompt, enter **dotnet restore .\RulesFunction.csproj**.
   ...
   ![Screenshot shows Visual Studio Code, Terminal window, and completed dotnet restore command.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/rules-engine/media/create-rules-engine-project/dotnet-restore-complete.png)
...
1. Confirm that the following items exist in your logic app project:
   ...
   ![Screenshot shows logic app workspace with function project and logic app project, now with the generated assemblies and other required files.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/rules-engine/media/create-rules-engine-project/generated-assemblies.png)
...
1. In your workspace, under **LogicApp**, expand the **<*workflow-name*>** node, open the shortcut menu for **workflow.json**, and select **Open Designer**.
   ...
   - The built-in [Request trigger named **When a HTTP request is received**](https://learn.microsoft.com/en-us/azure/../connectors/connectors-native-reqres).
   - The built-in action named **Call a local rules function in this logic app**.
   - The built-in [Response action named **Response**](https://learn.microsoft.com/en-us/azure/../connectors/connectors-native-reqres), which you use to reply to the caller only when you use the Request trigger.
...
1. Select the action named **Call a local rules function in this logic app**.
   ...
   ![Screenshot shows Visual Studio Code, workflow designer, and default workflow with trigger and actions.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/rules-engine/media/create-rules-engine-project/default-workflow.png)
...
1. Repeat the following steps to start the Azurite storage emulator *three* times: one time each for the following Azure Storage services:
   ...
   ![Screenshot shows Visual Studio Code taskbar with Azure Blob Service, Azure Queue Service, and Azure Table Service running.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/rules-engine/media/create-rules-engine-project/storage-services-running.png)

1. On the Visual Studio Code Activity Bar, select **Run and Debug**. (Keyboard: Ctrl+Shift+D)

   ![Screenshot shows Visual Studio Code Activity Bar with Run and Debug selected.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/rules-engine/media/create-rules-engine-project/run-debug.png)
...
1. From the **Run and Debug** list, select **Attach to logic app (LogicApp)**, if not already selected, and then select **Play** (green arrow).
   ...
   ![Screenshot shows Run and Debug list with Attach to logic app selected and Play button selected.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/rules-engine/media/create-rules-engine-project/attach-debugger-logic-app.png)
...
1. To set any breakpoints, in your function definition (**<*function-name*>.cs**) or workflow definition (**workflow.json**), find the line number where you want the breakpoint, and select the column to the left side, for example:
   ...
   ![Screenshot shows Visual Studio Code and the open function code file with a breakpoint set for a line in code.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/rules-engine/media/create-rules-engine-project/set-breakpoint.png)
...
1. To manually run the Request trigger in your workflow, open the workflow's **Overview** page.
   ...
   1. From your logic app project, open the **workflow.json** file's shortcut menu, and select **Overview**.
      ...
      ![Screenshot shows Visual Studio Code and workflow's Overview page opened.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/rules-engine/media/create-rules-engine-project/workflow-overview.png)
...
1. To review more information about the workflow run, select the finished run. Or, from the list next to the **Duration** column, select **Show run**.
   ...
   ![Screenshot shows Visual Studio Code and finished workflow run.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/rules-engine/media/create-rules-engine-project/workflow-run-history.png)

1. To deploy your logic apps with the Rules Engine project to Azure Logic Apps, follow the steps at [Prepare for deployment](https://learn.microsoft.com/en-us/azure/create-standard-workflows-visual-studio-code.md#prepare-for-deployment).
...
- [Create rules with the Microsoft Rules Composer](https://learn.microsoft.com/en-us/azure/logic-apps/create-rules)
- [Optimization for Azure Logic Apps Rules Engine execution](https://learn.microsoft.com/en-us/azure/logic-apps/rules-engine-optimization)
